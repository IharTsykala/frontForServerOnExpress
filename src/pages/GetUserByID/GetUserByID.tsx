...
const GetUserByID: React.FC<GetUserByProps> = ({user, dispatch, match}) => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault()
    const imgName = await Service.setImgUser(avatarForBack, idUserOwnerPage, user.role)
    await Service.editUser(idUserOwnerPage, { avatar: imgName, password: "" })
    if (idUserOwnerPage === user._id) dispatch(userLogIn(Object.assign({}, user, {avatar: imgName})))
  }  
