...
const PanelDialogs: React.FunctionComponent<PanelDialogsProps> = ({
  ...
  const getListDialogs = useCallback(
    async()=> {
      try {
        if (user._id) {
          ...
          if(allUsers[0]._id===undefined) dispatch(getAllUsersForSagasAction(user._id))
        }
      } catch (e) {
        console.log(e)
        setStateLoading(LoadingState.loaded)
  ...
  return (
    <>
      ...
      {(listDialogs.length > 0 &&
              !flagModalWindow &&listDialogs.map((dialog: any) => (        
          <ListItem key={dialog._id} button 
          onClick={()=>dispatch(currentDialogAction(dialog))}
          >
            <ListItemAvatar>
              <Avatar
                alt={`Avatar n°${dialog._id}`}
                src={(dialog.members.avatar&&`http://localhost:8080/images/users/${dialog.members._id}/${dialog.members.avatar}`)||
                (`http://localhost:8080/images/pattern-avatar.jpg`)
              }
              />
            </ListItemAvatar>
            <ListItemText id={dialog._id} primary={`${dialog.dialogName}`} />
  ...
  <ListItem key={secondUser._id} button
  onClick={()=>sendDialog(user._id, secondUser._id, secondUser.login)}
  >
    <ListItemAvatar>
      <Avatar
        alt={`Avatar n°${secondUser._id}`}
        src={(secondUser.avatar&&`http://localhost:8080/images/users/${secondUser._id}/${secondUser.avatar}`)||
        (`http://localhost:8080/images/pattern-avatar.jpg`)}
      />
    </ListItemAvatar>
    <ListItemText id={secondUser._id} primary={`${secondUser.login}`} />   
    {/* <Divider variant="inset" component="li" />          */}
  </ListItem>
)))    
     }
