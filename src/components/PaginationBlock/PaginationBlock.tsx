...
const PaginationBlock:  React.FunctionComponent<PaginationBlockProps> = ({
  ...
  setUsers
    ...
    // }, [input])
      ...
      const getUserAfterPaginationAndSearchAndFilter = async (numberPage:number, limitRender:number) => {
        let body = {}
        if(valueSearchBox.length>2) {
          body = Object.assign({}, body, {idLogInUser: user._id, valueSearchBox, numberPage, checked, limitRender})         
        } else body = Object.assign({}, body, {idLogInUser: user._id, valueSearchBox:'', numberPage, checked, limitRender})
        const listUsers = await Service.getUserAfterPaginationAndSearchAndFilter(body)
        setUsers(listUsers)
      }
